...
# List of random full names for students (ФИО)
random_full_names = [
...
def add_subjects_to_csv(filename, subjects):
    with open(filename, mode='a', encoding='utf-8', newline='') as file:
        ...
        for subject in subjects:
            writer.writerow(["", subject])  # Add an empty Full Name and the subject
...
def main():
    ...
    if choice == '1':
        ...
        # Shuffle the full names and repeat the subjects in a loop
    ...
    # Write the random full names and subjects to the CSV file
    with open("subjects.csv", mode='w', encoding='utf-8', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["ФИО", "Предмет"])  # Add the headers for Full Name and Subject
